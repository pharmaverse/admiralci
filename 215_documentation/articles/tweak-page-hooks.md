# Custom pkgdown tweak page hooks
...
[`help("getHook", "base")`](https://rdrr.io/r/base/userhooks.html).

The examples of issues that can be solved by tweak html file:

- [Links broken on pkgdown site
  \#13](https://github.com/pharmaverse/admiralci/issues/13)

### Custom hooks

Currently, [pkgdown](https://pkgdown.r-lib.org/) does not allow the set
up of any custom action. However, using custom hooks we can apply
changes during execution by using the `tweak_page()` function to execute
...
This part is executed automatically on GitHub CI. The developer need to
define only the hooks.

### Creating hooks

The `UserHook::admiralci::tweak_page` hook should be defined as
function. Please note that an error in function will stop the pkgdown
from building the site. The below code showing an example of
`tweak_page` function. The most convenient way to set up the hook is to
define them in the package `.onLoad` event.
