...
This job get latest R tag (checking dockerhub container registry). This
workflow is mainly here for `admiralci` developers to be warned if a new
version of R is released (then a manual update of `OLD_RELEASE` env
variable need to be done). Note: If latest R tag is different from
`OLD_RELEASE` this job will end-up will fail status.
...
1.  **List rocker/rstudio tags:** Request dockerhub registry to list all
    available tags. Then apply regexp pattern to retrieve only tags with
